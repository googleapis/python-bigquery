...
def test_get_model_field_headers():
    ...
    request.dataset_id = 'dataset_id_value'
    request.model_id = 'model_id_value'
    ...
    assert (
        ...
        'project_id=project_id_value&dataset_id=dataset_id_value&model_id=model_id_value',
    ) in kw['metadata']
...
def test_list_models_flattened():
    ...
    with mock.patch.object(
        ...
        mock_val = 'dataset_id_value'
        assert arg == mock_val
        assert wrappers.UInt32ValueRule().to_proto(args[0].max_results) == wrappers_pb2.UInt32Value(value=541)
...
def test_patch_model(request_type, transport: str = 'grpc'):
    client = ModelServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport=transport,
    )
