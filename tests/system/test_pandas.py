...
def test_list_rows_nullable_scalars_dtypes(bigquery_client, scalars_table, max_results):
    ...
    # TODO: Use Int64, because float64 can cause loss-of-precision.
    # https://github.com/googleapis/python-bigquery/issues/793
    assert df.dtypes["int64_col"].name == "float64"

    # TODO: Use boolean, which allows for null values.
    assert df.dtypes["bool_col"].name == "object"

    # TODO: Use datetime64[ns], as that is more natural for pandas data frames
    #       when dates are within allowed range.
    # https://pandas.pydata.org/pandas-docs/stable/user_guide/timeseries.html#overview
    assert df.dtypes["date_col"].name == "object"

    # TODO: Use timedelta64[ns], as that allows for additional operations, such
    #       as combination with DATE columns.
